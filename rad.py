...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'main': await start(update, context)
        ...
        u = get_user(uid)
        await query.edit_message_text(f"📈 **الصفقات النشطة:** `{u.get('trades', 0)}`", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data='main')]]))
    elif data == 'withdraw':
        await query.edit_message_text(f"📤 **لطلب السحب:**\nتواصل مع المدير المالي:\n{SUPPORT_LINK}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data='main')]]))
        ...
        if data.startswith('ok_'):
            ...
            await query.edit_message_caption("✅ **تم الشحن وتفعيل الصفقة!**")
            try: await context.bot.send_message(chat_id=int(tid), text=f"🎉 **مبروك!** تم تأكيد إيداعك بقيمة `{amt}`.")
            except: pass
...
async def on_receipt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        user = update.effective_user
        await update.message.reply_text("⏳ **تم استلام إيصالك، جاري التدقيق..**")
        # أزرار الإدارة المكتملة
...
if __name__ == '__main__':
    ...
    logger.info("🔥 Dragon Final System is Ready!")
