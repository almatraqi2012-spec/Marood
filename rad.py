...
# تشفير بيانات المونقو (البيانات المأخوذة من صورتك الأخيرة)
u = urllib.parse.quote_plus('Abduh')
p = urllib.parse.quote_plus('Abduh2026')
# الرابط المطور مع authSource=admin لفتح القفل
...
# الاتصال بقاعدة البيانات مع اختبار الاتصال
try:
...
# --- 🔘 معالج كافة الأزرار (أزرار المستخدم والمالك) ---
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- 🛠️ أزرار المالك (التأكيد والرفض) ---
    elif data.startswith('ok_'):
        if uid not in ADMINS_LIST: return
        try:
            _, curr, amt, target = data.split('_')
            update_balance(target, curr, amt)
            await query.edit_message_caption(caption=f"✅ تم تأكيد إيداع {amt} {curr}\nللمستخدم: `{target}`")
            await context.bot.send_message(chat_id=int(target), text=f"🎉 أبشر.. تم إيداع `{amt}` {curr} في محفظتك بنجاح!")
        except Exception as e: logger.error(f"Error in OK: {e}")
    elif data.startswith('no_'):
        if uid not in ADMINS_LIST: return
        target = data.split('_')[1]
        await query.edit_message_caption(caption="❌ تم رفض الإيصال من قبل الإدارة.")
        try: await context.bot.send_message(chat_id=int(target), text="❌ نعتذر منك، تم رفض الإيصال من قبل الإدارة.")
        except: pass

# --- 📸 معالج الإيصالات وأزرار المالك ---
async def handle_receipt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        user = update.effective_user
        await update.message.reply_text("✅ تم استلام الإيصال، جاري المراجعة...")
        # تعريف الأزرار بالتنسيق الصحيح (ok_العملة_المبلغ_الايدي)
        ...
        for adm in ADMINS_LIST:
            try: await context.bot.send_photo(chat_id=adm, photo=update.message.photo[-1].file_id, 
...
if __name__ == '__main__':
    # تشغيل خادم الويب في الخلفية لضمان استمرار Render
    Thread(target=run_server, daemon=True).start()
    
    # بناء التطبيق
    app = Application.builder().token(TOKEN).concurrent_updates(True).build()
    
    # إضافة المعالجات بالترتيب الصحيح (الأوامر أولاً)
    app.add_handler(CommandHandler('start', start))
    app.add_handler(CallbackQueryHandler(button_handler))
    app.add_handler(MessageHandler(filters.PHOTO, handle_receipt))
    
    logger.info("🚀 الانطلاق النهائي.. تم إصلاح كافة الميزات!")
    # تنظيف الطابور لضمان استجابة Start الفورية
