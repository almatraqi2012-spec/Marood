...
# --- 🔐 الربط الذهبي للمونقو (تم إضافة مصدر المصادقة) ---
u = urllib.parse.quote_plus('Abduh')
p = urllib.parse.quote_plus('Abduh2026')
# الإضافة الجوهرية هنا هي authSource=admin لتطابق صلاحياتك في الصورة
MONGO_URI = f"mongodb+srv://{u}:{p}@cluster0.0a4wefx.mongodb.net/DragonFinal?authSource=admin&retryWrites=true&w=majority"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [
        ...
        [InlineKeyboardButton("💬 التواصل مع الإدارة", url="https://t.me/HCICICVICIF9")]
    ]
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'main': await start(update, context)
        ...
        await query.edit_message_text(f"📊 **محفظتك:**\n\n🇸🇦: `{u_data['bal_sar']}` ﷼\n🇺🇸: `{u_data['bal_usd']}` $", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data='main')]]), parse_mode='Markdown')
        ...
        if data.startswith('ok_'):
            field = "bal_sar" if parts[1] == 'sr' else "bal_usd"
            users_col.update_one({"uid": int(parts[3])}, {"$inc": {field: float(parts[2])}})
            await query.edit_message_caption("✅ تم التأكيد والإيداع")
            try: await context.bot.send_message(int(parts[3]), f"🎉 تم إيداع {parts[2]} في محفظتك!")
            except: pass
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for adm in ADMINS_LIST:
        try: await context.bot.send_photo(adm, update.message.photo[-1].file_id, caption=f"🔔 إيداع من: {user.first_name}", reply_markup=InlineKeyboardMarkup(kb))
        except: pass
