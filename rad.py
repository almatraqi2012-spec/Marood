...
flask_app = Flask('')
@flask_app.route('/')
def home(): return "🚀 Dragon System Active"
...
async def cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'main': await start(update, context)
        ...
        if data.startswith('ok'):
            ...
            # ربط العملة بالمفتاح الصحيح لضمان ظهور الرصيد
            target_key = "sar" if c == "sr" else "usd"
            if users_col: users_col.update_one({"uid": int(tid)}, {"$inc": {target_key: float(a), "t": 1}}, upsert=True)
            await q.edit_message_caption("✅ تم شحن الرصيد بنجاح!")
            try: await context.bot.send_message(tid, f"🎊 مبروك! تم شحن `{a}` {'﷼' if target_key=='sar' else '$'} في حسابك.")
            except: pass
