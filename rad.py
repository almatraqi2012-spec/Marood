...
flask_app = Flask('')
@flask_app.route('/')
def home(): return "🚀 Sahm Pro: Active"
...
# مبالغ الاستثمار
SAR_VALS = ["1000", "5000", "10000", "50000"]
USD_VALS = ["300", "1000", "5000", "10000"]
...
async def cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- 🛡️ نظام الإدارة (الإصلاح الجذري هنا) ---
    elif data.startswith(('v', 'x')):
        if uid not in ADMINS: return
        # فك التشفير: v|العملة|المبلغ|المعرف
        try:
            p = data.split('|')
            ...
            if act == 'v':
                ...
                await q.edit_message_caption(f"✅ تم اعتماد {amt}")
            ...
            logger.error(f"Error in admin buttons: {e}")
...
async def photo_h(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ⚡ اختصار البيانات لأقصى حد لضمان استجابة الزر
    def b(c, v): return InlineKeyboardButton(f"{v} {('﷼' if c=='sr' else '$')}", callback_data=f"v|{c}|{v}|{u.id}")

    kb = [[b('sr', '1000'), b('sr', '5000')], [b('sr', '10000'), b('sr', '50000')],
          [b('us', '300'), b('us', '1000')], [b('us', '5000'), b('us', '10000')],
          [InlineKeyboardButton("🚫 رفض", callback_data=f"x|0|0|{u.id}")]]
    
    for adm in ADMINS:
        await context.bot.send_photo(adm, update.message.photo[-1].file_id, 
                                   caption=f"🔔 من: {u.first_name}\nID: `{u.id}`", 
